...
fn resolver_actor() -> Actor {
    Actor { id: "resolver:insee".into(), kind: ActorKind::Resolver }
}
...
fn item(i: &str) -> RowPath {
    RowPath::root().child(PathSeg { group: GroupId::new("g1"), item: ItemId::new(i) })
}
...
fn checkpoint(log: &RecordLog, at: usize, scope: RowPath) -> Checkpoint {
    Checkpoint {
        ...
        frozen_columns: ["name", "raison_sociale", "adresse"].into_iter().map(ColumnId::new).collect(),
...
fn base() -> (RecordLog, Checkpoint) {
    ...
    log.append(draft(human("a1"), 0, 0, Origin::Entered, vec![set("name", "Dupont")])).unwrap();
...
fn a_checkpoint_naming_an_unknown_entry_reports_only_that() {
    ...
    assert_eq!(validate_after_checkpoint(&log, &cp, None), vec![CheckpointViolation::UnknownEntry]);
}

#[test]
fn a_superseding_checkpoint_must_name_a_later_entry_of_this_log() {
    let (mut log, cp) = base();
    log.append(draft(human("a1"), 1, 1, Origin::Entered, vec![set("name", "X")])).unwrap();
    log.append(draft(human("a1"), 2, 2, Origin::Entered, vec![set("annotation", "ok")])).unwrap();
...
fn the_superseding_entry_is_judged_under_the_old_regime_and_nothing_after_it_is() {
    ...
    log.append(draft(human("a1"), 1, 1, Origin::Entered, vec![set("name", "edited")])).unwrap();
    log.append(draft(human("a1"), 2, 2, Origin::Entered, vec![set("adresse", "resubmitted")])).unwrap();
    let second = checkpoint(&log, 2, RowPath::root());
    log.append(draft(human("a1"), 3, 3, Origin::Entered, vec![set("name", "after")])).unwrap();

    assert_eq!(
        validate_after_checkpoint(&log, &first, Some(&second)),
        vec![illegal(1, &["name"], &[]), illegal(2, &["adresse"], &[])]
    );
    assert_eq!(validate_after_checkpoint(&log, &second, None), vec![illegal(3, &["name"], &[])]);
    // Without the superseder, the first checkpoint sees everything.
    assert_eq!(
        validate_after_checkpoint(&log, &first, None),
        vec![illegal(1, &["name"], &[]), illegal(2, &["adresse"], &[]), illegal(3, &["name"], &[])]
    );
}
...
fn an_expected_write_must_match_the_bound_versions_exactly() {
    ...
    log.append(draft(resolver_actor(), 1, 1, Origin::Derived(other_resolver), vec![set("adresse", "x")]))
        .unwrap();
    log.append(draft(resolver_actor(), 2, 2, Origin::Derived(other_mapping), vec![set("adresse", "y")]))
        .unwrap();
    // The exact versions: legal.
    log.append(draft(resolver_actor(), 3, 3, Origin::Derived(derivation()), vec![set("adresse", "z")]))
        .unwrap();
    assert_eq!(
...
fn an_expected_write_must_stay_within_the_expected_scope() {
    ...
    log.append(draft(resolver_actor(), 1, 1, Origin::Derived(derivation()), vec![set("adresse", "root")]))
        .unwrap();
    log.append(draft(resolver_actor(), 2, 2, Origin::Derived(derivation()), vec![set_at("adresse", item("i2"), "sibling")]))
        .unwrap();
    log.append(draft(resolver_actor(), 3, 3, Origin::Derived(derivation()), vec![set_at("adresse", item("i1"), "own")]))
        .unwrap();
    ...
    assert_eq!(
        validate_after_checkpoint(&log, &cp, None),
        vec![illegal(1, &["adresse"], &[]), illegal(2, &["adresse"], &[]), illegal(4, &["adresse"], &[])]
    );
}
...
fn only_a_resolver_actor_with_a_derived_origin_can_be_expected() {
    ...
    let (mut log, cp) = base();
    log.append(draft(human("a1"), 1, 1, Origin::Derived(derivation()), vec![set("adresse", "x")]))
        .unwrap();
    log.append(draft(resolver_actor(), 2, 2, Origin::Entered, vec![set("adresse", "y")])).unwrap();
    // `overridden` carrying the same derivation is not `derived` either.
    log.append(draft(
        resolver_actor(),
        3,
        3,
        Origin::Overridden { superseded: Some(derivation()) },
        vec![set("adresse", "z")],
    ))
    .unwrap();
    assert_eq!(
        validate_after_checkpoint(&log, &cp, None),
        vec![illegal(1, &["adresse"], &[]), illegal(2, &["adresse"], &[]), illegal(3, &["adresse"], &[])]
    );
}
...
fn a_mixed_write_names_every_frozen_column_and_group_it_touched() {
    ...
    log.append(draft(
        ...
        vec![
            ...
            Op::AddItem { group: GroupId::new("g1"), parent: RowPath::root(), item: ItemId::new("i1"), at: 0 },
            Op::AddItem { group: GroupId::new("other"), parent: RowPath::root(), item: ItemId::new("i1"), at: 0 },
        ],
    ))
    .unwrap();
    ...
    log.append(draft(
        ...
        Origin::Entered,
        vec![
            Op::Unset { column: ColumnId::new("name"), path: RowPath::root() },
            Op::Reorder { group: GroupId::new("g1"), parent: RowPath::root(), order: vec![ItemId::new("i1")] },
        ],
    ))
    .unwrap();
    log.append(draft(
