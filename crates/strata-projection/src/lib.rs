...
use strata_core::primitives::Decimal;
use strata_core::{ColumnId, OptionId};
use strata_schema::{
    Arity, Cast, CastError, ColumnInfo, NomenclatureTable, ScalarType, Schema,
    SchemaIndex, column_cast, nomenclature_rows,
};
use strata_value::{CellState, CellValue, RecordValues, Scalar};
...
pub fn project(
    ...
    for (column, rinfo) in &reader_index.columns {
        ...
        let mut column_report = ColumnReport {
            ...
            } else if cast.class() == strata_schema::CastClass::Identity {
    ...
    for (addr, items) in &values.items {
        let keep = reader_index.groups.get(&addr.group).is_some_and(|g| {
            g.cardinality == strata_schema::Cardinality::Many
                && addr
                    .parent
                    .segments()
                    .iter()
...
fn project_scalar(
    ...
    match (from, to) {
        ...
        (T::Text, T::Date) => text(scalar, |s| {
            strata_core::primitives::Date::parse(s).ok().map(Scalar::Date)
        }),
        (T::Text, T::Datetime) => text(scalar, |s| {
            strata_core::primitives::Instant::parse(s)
                .ok()
                .map(Scalar::Datetime)
        }),
