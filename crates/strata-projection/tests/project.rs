use strata_core::primitives::{Decimal, Instant};
use strata_core::{ColumnId, GroupId, ItemId, OptionId, PathSeg, RowPath};
use strata_projection::{ColumnStatus, project};
use strata_schema::{
    Arity, Cardinality, Column, Element, Group, NomenclatureRef, OptionRow,
    ScalarType, Schema,
};
use strata_value::{CellAddr, CellState, CellValue, ItemsAddr, RecordValues, Scalar};
...
fn widening_casts_convert_values() {
    ...
    v.cells.insert(
        addr("d"),
        one(Scalar::Date(
            strata_core::primitives::Date::parse("2026-08-16").unwrap(),
        )),
    );
    v.cells
