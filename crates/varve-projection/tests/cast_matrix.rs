...
use varve_schema::{
    Arity, AttachmentConstraints, CastClass, Column, Element, NomenclatureRef,
    NomenclatureTable, OptionRow, ScalarType, Schema, Unit, scalar_cast,
};
use varve_value::{AttachmentRef, CellAddr, CellState, CellValue, Feature, RecordValues, Scalar, check};
...
fn addr() -> CellAddr {
    CellAddr { column: ColumnId::new("c"), path: RowPath::root() }
}
...
fn inline(rows: &[(&str, &str)]) -> ScalarType {
    ScalarType::Enum(NomenclatureRef::Inline(rows.iter().map(|(i, l)| row(i, l)).collect()))
}
...
fn values_for(ty: &ScalarType) -> Vec<Scalar> {
    use ScalarType::*;
    match ty {
        Text => ["12", "1.5", "true", "2024-02-29", "2024-02-29T10:00:00Z", "Oui", "Ain", "abc"]
            .into_iter()
            .map(|s| Scalar::Text(s.into()))
            .collect(),
        Boolean => vec![Scalar::Boolean(true), Scalar::Boolean(false)],
        Integer(_) => vec![Scalar::Integer(7), Scalar::Integer(-3), Scalar::Integer(1_440)],
        Decimal(_) => ["1.5", "12", "7"]
            .into_iter()
            .map(|s| Scalar::Decimal(varve_core::primitives::Decimal::parse(s).unwrap()))
            .collect(),
        Date => vec![Scalar::Date(varve_core::primitives::Date::parse("2024-02-29").unwrap())],
        Datetime => vec![
        ...
        Enum(nref) => varve_schema::nomenclature_rows(nref, &table())
            ...
            .collect(),
        Attachment(c) => [file("a.pdf", "application/pdf", 500), file("b.png", "image/png", 5_000)]
            .into_iter()
            .filter(|s| match s {
...
fn every_value_of_every_type_conforms_to_its_own_column() {
    ...
    for ty in types() {
        ...
        for v in vs {
            ...
            assert_eq!(check(&values(one(&v)), &schema, &n), vec![], "{v:?} in {ty:?}");
        }
    }
}
    ...
    for from in &types {
        for to in &types {
            ...
            for value in values_for(from) {
                ...
                assert_eq!(col.cells_projected + col.cells_failed, u64::from(cast.possible), "{ctx}");
...
fn empty_cells_pass_through_every_possible_cast() {
    ...
    for from in &types {
        for to in &types {
            ...
            let col = &p.report.columns[&ColumnId::new("c")];
            if cast.possible {
                assert_eq!(p.values.cells.get(&addr()), Some(&CellState::Empty), "{from:?} → {to:?}");
                assert_eq!((col.cells_projected, col.cells_lossy, col.cells_failed), (1, 0, 0));
            } else {
                assert!(p.values.cells.is_empty(), "{from:?} → {to:?}");
                assert_eq!((col.cells_projected, col.cells_lossy, col.cells_failed), (0, 0, 0));
            }
        }
    }
}
...
fn loss_is_counted_per_cell_where_it_actually_happens() {
    ...
    let lossy_of = |from: ScalarType, to: ScalarType, value: Scalar| {
        let p = project(&values(one(&value)), &single(from, Arity::One), &single(to, Arity::One), &n)
            .unwrap();
        let col = &p.report.columns[&ColumnId::new("c")];
    ...
    assert_eq!(lossy_of(ScalarType::Datetime, ScalarType::Date, midnight), (0, date.clone()));
    assert_eq!(lossy_of(ScalarType::Datetime, ScalarType::Date, ten), (1, date));
    // Unit removed (§2.14): the bytes survive, the meaning does not —
    // every cell counts.
    assert_eq!(
        lossy_of(ScalarType::Integer(Some(Unit::Day)), ScalarType::Integer(None), Scalar::Integer(7)),
        (1, one(&Scalar::Integer(7)))
    );
    // Unit added: free, and clean.
    assert_eq!(
        lossy_of(ScalarType::Integer(None), ScalarType::Integer(Some(Unit::Day)), Scalar::Integer(7)),
    ...
    assert_eq!(lossy_of(published("cog", 1), cog_inline.clone(), ain.clone()), (1, one(&ain)));
    assert_eq!(lossy_of(cog_inline, published("cog", 2), ain.clone()), (0, one(&ain)));
    // Same published nomenclature, higher version: free (append-only).
    assert_eq!(lossy_of(published("cog", 1), published("cog", 2), ain.clone()), (0, one(&ain)));
}
...
fn unit_conversion_is_exact_or_fails_the_cell() {
    ...
    assert_eq!(run(day.clone(), week.clone(), Scalar::Integer(7)), (Some(one(&Scalar::Integer(1))), 0));
    assert_eq!(run(day.clone(), week.clone(), Scalar::Integer(-3)), (None, 1));
    assert_eq!(run(week, day, Scalar::Integer(-3)), (Some(one(&Scalar::Integer(-21))), 0));
    ...
    assert_eq!(run(minute.clone(), hour.clone(), d("90")), (Some(one(&d("1.5"))), 0));
...
fn arity_matrix() {
    ...
    for ty in types() {
        ...
        assert_eq!(p.report.columns[&ColumnId::new("c")].cells_lossy, 0, "{ctx}");
    }
...
fn attachment_constraint_changes_recheck_the_claims() {
    ...
    assert_eq!((cell, col.status.clone(), col.cells_failed), (None, ColumnStatus::Cast, 1));
    ...
    for (from, value) in [(pdf_only.clone(), pdf.clone()), (images.clone(), png.clone())] {
        let (cell, col) = run(from, any.clone(), value.clone());
        assert_eq!((cell, col.status, col.cells_failed, col.cells_lossy), (Some(one(&value)), ColumnStatus::Cast, 0, 0));
    }
    ...
    assert_eq!((cell, col.status, col.cells_failed), (None, ColumnStatus::Cast, 1));
    ...
    assert_eq!((cell, col.status), (Some(one(&pdf)), ColumnStatus::Identity));
}
