...
fn seg(group: &str, item: &str) -> PathSeg {
    PathSeg { group: g(group), item: i(item) }
}
...
fn set(column: &str, path: RowPath, state: CellState) -> Op {
    Op::Set { column: ColumnId::new(column), path, state }
}

fn add(group: &str, parent: RowPath, item: &str, at: usize) -> Op {
    Op::AddItem { group: g(group), parent, item: i(item), at }
}

fn remove(group: &str, parent: RowPath, item: &str) -> Op {
    Op::RemoveItem { group: g(group), parent, item: i(item) }
}

fn reorder(group: &str, parent: RowPath, order: &[&str]) -> Op {
    Op::Reorder { group: g(group), parent, order: order.iter().map(|s| i(s)).collect() }
}

fn items(group: &str, parent: RowPath) -> ItemsAddr {
    ItemsAddr { group: g(group), parent }
}

fn cell(column: &str, path: RowPath) -> CellAddr {
    CellAddr { column: ColumnId::new(column), path }
}
...
fn sample() -> RecordValues {
    ...
    v.cells.insert(cell("name", RowPath::root()), text("Dupont"));
    v.items.insert(items("g1", RowPath::root()), vec![i("i1"), i("i2")]);
    for item in ["i1", "i2"] {
        v.cells.insert(cell("c1", RowPath::root().child(seg("g1", item))), text(item));
    }
    let i1 = RowPath::root().child(seg("g1", "i1"));
    v.items.insert(items("g2", i1.clone()), vec![i("j1")]);
    v.cells.insert(cell("c2", i1.child(seg("g2", "j1"))), text("nested"));
    v
}
...
fn refuse(op: Op, err: ApplyError) {
    ...
    assert_eq!(v, before, "a refused op must leave the values unchanged: {op:?}");
}

#[test]
fn item_exists() {
    for at in [0, 1, 2] {
        refuse(add("g1", RowPath::root(), "i1", at), ApplyError::ItemExists(g("g1"), i("i1")));
        refuse(add("g1", RowPath::root(), "i2", at), ApplyError::ItemExists(g("g1"), i("i2")));
    }
}
...
fn unknown_item() {
    ...
    refuse(remove("g9", RowPath::root(), "i1"), ApplyError::UnknownItem(g("g9"), i("i1")));
    refuse(
        remove("g2", RowPath::root().child(seg("g1", "i2")), "j1"),
        ApplyError::UnknownItem(g("g2"), i("j1")),
    );
    // A list exists, the item is not in it.
    refuse(remove("g1", RowPath::root(), "ghost"), ApplyError::UnknownItem(g("g1"), i("ghost")));
}

#[test]
fn bad_index() {
    // Two items: 0, 1 and 2 (append) are fine; 3 is out of bounds.
    refuse(add("g1", RowPath::root(), "i3", 3), ApplyError::BadIndex(g("g1"), 3));
    refuse(add("g1", RowPath::root(), "i3", usize::MAX), ApplyError::BadIndex(g("g1"), usize::MAX));
    // A group with no list yet: only 0 is a valid index.
    refuse(add("g9", RowPath::root(), "x", 1), ApplyError::BadIndex(g("g9"), 1));
}

#[test]
fn bad_reorder() {
    // No list to reorder.
    refuse(reorder("g9", RowPath::root(), &["a"]), ApplyError::BadReorder(g("g9")));
    refuse(reorder("g9", RowPath::root(), &[]), ApplyError::BadReorder(g("g9")));
    // Different length: shorter and longer.
    refuse(reorder("g1", RowPath::root(), &["i1"]), ApplyError::BadReorder(g("g1")));
    refuse(reorder("g1", RowPath::root(), &["i2", "i1", "i3"]), ApplyError::BadReorder(g("g1")));
    // Same length, different elements.
    refuse(reorder("g1", RowPath::root(), &["i1", "i3"]), ApplyError::BadReorder(g("g1")));
    // Repeated element.
    refuse(reorder("g1", RowPath::root(), &["i1", "i1"]), ApplyError::BadReorder(g("g1")));
}
...
fn empty_list() {
    ...
    refuse(
        set("tags", RowPath::root(), CellState::Value(CellValue::Many(vec![]))),
        ApplyError::EmptyList(ColumnId::new("tags")),
    );
    // Even over an existing cell: the previous value survives.
    refuse(
        set("name", RowPath::root(), CellState::Value(CellValue::Many(vec![]))),
...
fn add_item_at_len_appends() {
    ...
    assert_eq!(v.items[&items("g1", RowPath::root())], vec![i("i1"), i("i2"), i("i3")]);
...
fn removing_the_last_item_removes_the_list() {
    ...
    assert_eq!(v.items[&items("g1", RowPath::root())], vec![i("i1"), i("i2")]);
}
...
fn remove_item_cascades_through_nested_lists() {
    ...
    assert!(v.cells.contains_key(&cell("c1", RowPath::root().child(seg("g1", "i2")))));
...
fn unset_of_an_absent_cell_is_a_no_op() {
    let before = sample();
    let mut v = before.clone();
    apply(&mut v, &Op::Unset { column: ColumnId::new("nope"), path: RowPath::root() }).unwrap();
    assert_eq!(v, before);
    // And unset of a present cell removes it (back to absent, not Empty).
    apply(&mut v, &Op::Unset { column: ColumnId::new("name"), path: RowPath::root() }).unwrap();
    assert!(!v.cells.contains_key(&cell("name", RowPath::root())));
}
...
fn set_overwrites_and_reorder_to_the_same_order_is_ok() {
    ...
    assert_eq!(v.items[&items("g1", RowPath::root())], vec![i("i2"), i("i1")]);
